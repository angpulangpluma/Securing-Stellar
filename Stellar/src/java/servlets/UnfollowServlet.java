...
public class UnfollowServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    try {
	        ...
	        String query = "DELETE FROM followingdata WHERE followingId = "
		    				+profileId+" AND userId = "
		    				+id+";";
   
	        int update = stmt.executeUpdate(query);
	        query = "DELETE FROM followerdata WHERE followerId = "
    				+id+" AND userId = "
    				+profileId+";";

	        update = stmt.executeUpdate(query);	//for data manipulation, ERROR CHECK update	
			stmt2 = conn.createStatement();
			
			query = "SELECT * FROM userdata WHERE userId = " + profileId;
